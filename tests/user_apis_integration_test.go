...
func TestIntegrationRegisterUserWithWeakPassword(t *testing.T) {
	...
	// ACT
	// Send an HTTP POST request to create a user using the test server
	...
	expectedRequestBody := `{"type":"REQUEST_DATA.INVALID","message":"invalid request data","details":{"password":"password not strong"}}`
	assert.Equal(t, http.StatusUnprocessableEntity, resp.StatusCode, "should return 422 status code")
	assert.Equal(t, expectedRequestBody, string(responseBody), "should return error details in the response body")
}
...
func TestIntegrationRegisterUserWithUsedEmail(t *testing.T) {
	...
	// ASSERT
	responseBody, _ := io.ReadAll(resp.Body)
	expectedRequestBody := fmt.Sprintf(`{"type":"USER.ALREADY_EXISTS","message":"user with the email '%s' already exists","details":null}`, email)
	assert.Equal(t, http.StatusBadRequest, resp.StatusCode, "should return 400 status code")
	assert.Equal(t, expectedRequestBody, string(responseBody), "should return the error details in the response body")
}
